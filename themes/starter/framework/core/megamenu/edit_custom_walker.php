...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu {
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        if ( ! empty( $item->_invalid ) ) {
            ...
            $title = sprintf( esc_html__( '%s (Invalid)','starter' ), $item->title );
        } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf( esc_html__('%s (Pending)','starter'), $item->title );
        }
                ...
                <dt class="menu-item-handle">
                    ...
                    <span class="item-controls">
                        ...
                        <span class="item-order hide-if-js">
                            ...
                            ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','starter'); ?>">&#8593;</abbr></a>
                            |
                            <a href="<?php
                                echo wp_nonce_url(
                        ...
                        <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item','starter'); ?>" href="<?php
                        ...
                        ?>"><span class="screen-reader-text"><?php esc_html_e( 'Edit Menu Item','starter' ); ?></span></a>
                    </span>
                </dt>
            </dl>
            ...
            <div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
                ...
                <p class="description description-thin">
                    <label for="menu-item-megamenu_sublabel-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Sub Label', 'starter' ); ?><br />
                ...
                <p class="description description-thin sublabel-color">
                    <label for="menu-item-megamenu_sublabel_color-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Sub Label Color', 'starter' ); ?><br />
                ...
                <div class="haru-mega-menu-icon">
                    <label for="edit-menu-item-megamenu_icon-<?php echo esc_attr($item_id); ?>"><?php esc_html_e( 'Icon','starter' ); ?></label>
                    <div class="haru-icon">
                        <span class="icon">
                        ...
                        <div class="mega-entry list-entry-<?php echo esc_attr($item_id); ?>"  data-id="<?php echo esc_attr($item_id); ?>">
                            <span class="megamenu-search">
                                <input type="text" class="box-search search-<?php echo esc_attr($item_id); ?>" placeholder="<?php echo esc_attr__( 'Ex: balance-scale', 'starter' ); ?>" />
                        ...
                        <div class="mega-entry box-set color-<?php echo esc_attr($item_id); ?>">
                            ...
                            <div class="size">
                                <label for="edit-menu-item-megamenu_icon_size-<?php echo esc_attr($item_id); ?>"><?php esc_html_e( 'Size', 'starter' ); ?></label>
                            ...
                            <div>
                                <label for="edit-menu-item-megamenu_icon_alignment-<?php echo esc_attr($item_id); ?>"><?php esc_html_e( 'Icon Alignment', 'starter' ); ?></label>
                                <select class="select_alignment" id="edit-menu-item-megamenu_icon_alignment-<?php echo esc_attr($item_id); ?>" name="menu-item-megamenu_icon_alignment[<?php echo esc_attr($item_id); ?>]">
                                    <option value="left"<?php selected( $item->megamenu_icon_alignment, 'left' ); ?>><?php esc_html_e( 'Left', 'starter' ); ?></option>
                                    <option value="right"<?php selected( $item->megamenu_icon_alignment, 'right' ); ?>><?php esc_html_e( 'Right', 'starter' ); ?></option>
                                    <option value="center"<?php selected( $item->megamenu_icon_alignment, 'center' ); ?>><?php esc_html_e( 'Center', 'starter' ); ?></option>
                                </select>
                            </div>
                ...
                <!-- ENABLE MEGAMENU -->
                <p class="megamenu-status description description-wide">
                    ...
                    <?php esc_html_e( 'Enable megamenu','starter' );    ?>
                    </label>
                </p>
                ...
                <p class="megamenu-style description description-wide enable_megamenu_child-<?php echo esc_attr( $item_id ); ?>">
                    ...
                    <?php esc_html_e( 'Megamenu style', 'starter' ); ?>
                    <br/>
                    <select id="menu-item-megamenu_style-<?php echo esc_attr($item->ID); ?>" name="menu-item-megamenu_style[<?php echo esc_attr($item->ID); ?>]" class="widefat code edit-menu-item-custom">
                        <option <?php selected( $item->megamenu_style, 'menu_style_column' ); ?> value="menu_style_column"><?php esc_html_e( 'Column', 'starter' ); ?></option>
                        <option <?php selected( $item->megamenu_style, 'menu_style_tab' ); ?> value="menu_style_tab"><?php esc_html_e( 'Tab', 'starter' ); ?></option>
                    </select>
                </p>
                ...
                <p class="megamenu-columns description description-wide megamenu-child-options enable_megamenu_child-<?php echo esc_attr( $item_id ); ?>">
                    <label for="menu-item-megamenu_columns-<?php echo esc_attr($item->ID); ?>">
                        <?php esc_html_e( 'Megamenu columns', 'starter' ); ?>
                        <br/>
                        <select id="menu-item-megamenu_columns-<?php echo esc_attr($item->ID); ?>" name="menu-item-megamenu_columns[<?php echo esc_attr($item->ID); ?>]" class="widefat code edit-menu-item-custom">
                            <option <?php selected( $item->megamenu_col, 'columns-2' ) ?> value="columns-2"><?php esc_html_e( 'Two', 'starter' ) ?></option>
                            <option <?php selected( $item->megamenu_col, 'columns-3' ) ?> value="columns-3"><?php esc_html_e( 'Three', 'starter' ) ?></option>
                            <option <?php selected( $item->megamenu_col, 'columns-4' ) ?> value="columns-4"><?php esc_html_e( 'Four', 'starter' ) ?></option>
                            <option <?php selected( $item->megamenu_col, 'columns-5' ) ?> value="columns-5"><?php esc_html_e( 'Five', 'starter' ) ?></option>
                            <option <?php selected( $item->megamenu_col, 'columns-6' ) ?> value="columns-6"><?php esc_html_e( 'Six', 'starter' ) ?></option>
                        </select>
                    </label>
                </p>    
                ...
                <p class="megamenu-background-image description description-wide megamenu-child-options enable_megamenu_child-<?php echo esc_attr( $item_id ); ?>">
                    <label for="menu-item-megamenu_background_image-<?php echo esc_attr($item->ID); ?>">
                        <?php esc_html_e( 'Background Image', 'starter' ); ?>
                        <br/>
                ...
                <p class="haru-mega-menu-width description description-wide enable_megamenu_child-<?php echo esc_attr( $item_id ); ?>">
                    ...
                    <?php esc_html_e( 'Sub menu fullwidth? (only use with Column style)','starter' );    ?>
                    </label>
                </p>
                ...
                <p class="haru-mega-menu-heading description description-wide enable_megamenu_child-<?php echo esc_attr( $item_id ); ?>">
                    ...
                    <?php esc_html_e( 'Hide Mega menu heading?','starter' );    ?>
                    </label>
                </p>
                ...
                <p class="haru-mega-menu-widgetarea description description-wide enable_megamenu_child-<?php echo esc_attr( $item_id ); ?>">
                    <label for="edit-menu-item-megamenu_widgetarea-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Mega Menu Widget Area', 'starter' ); ?>
                        <select id="edit-menu-item-megamenu_widgetarea-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-custom" name="menu-item-megamenu_widgetarea[<?php echo esc_attr($item_id); ?>]">
                            <option value="0"><?php esc_html_e( 'Select Widget Area', 'starter' ); ?></option>
                            <?php
                ...
                <p class="megamenu-columns-tab description description-wide megamenu-child-options enable_megamenu_child-<?php echo esc_attr( $item_id ); ?>">
                    <label for="menu-item-megamenu_columns_tab-<?php echo esc_attr($item->ID); ?>">
                        <?php esc_html_e( 'Tab columns (only use with tab style)', 'starter' ); ?>
                        <br/>
                        <select id="menu-item-megamenu_columns_tab-<?php echo esc_attr($item->ID); ?>" name="menu-item-megamenu_columns_tab[<?php echo esc_attr($item->ID); ?>]" class="widefat code edit-menu-item-custom">
                            <option <?php selected( $item->megamenu_col_tab, 'columns-1' ) ?> value="columns-1"><?php esc_html_e( 'One', 'starter' ) ?></option>
                            <option <?php selected( $item->megamenu_col_tab, 'columns-2' ) ?> value="columns-2"><?php esc_html_e( 'Two', 'starter' ) ?></option>
                            <option <?php selected( $item->megamenu_col_tab, 'columns-3' ) ?> value="columns-3"><?php esc_html_e( 'Three', 'starter' ) ?></option>
                            <option <?php selected( $item->megamenu_col_tab, 'columns-4' ) ?> value="columns-4"><?php esc_html_e( 'Four', 'starter' ) ?></option>
                        </select>
                    </label>
                </p>            
                <?php
                ...
                <div class="menu-item-actions description-wide submitbox">
                    <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                        <p class="link-to-original">
                            <?php printf( esc_html__('Original: %s','starter'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                        </p>
                    <?php endif; ?>
                    ...
                    ); ?>"><?php esc_html_e('Remove','starter'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
