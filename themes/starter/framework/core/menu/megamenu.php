        ...
        public function generateSubMegaMenu( $item, $haru_mega_profile ) {
            if ( $haru_mega_profile ) {
                ...
                if ( $posts && isset($posts[0]) ) {
                    $post = $posts[0];
                    $content = apply_filters( 'haru_render_post_builder', do_shortcode( $post->post_content ), $post->ID);
                    $width  = $item->haru_width ? 'style="width: ' . $item->haru_width . 'px;"' : '';

                    return '<ul class="sub-menu ' . esc_attr( $post->post_name ) . '" ' . $width . '><div class="mega-menu-content">' . $content . '</div></ul>';
                }
            }
