...
if ( ! class_exists( 'Haru_Term_Meta' ) ):
	...
	final class Haru_Term_Meta {
		...
		public function load_scripts_styles() {
			...
			wp_enqueue_style( 'term-meta-style', HARU_STARTER_CORE_URL . 'includes/term-meta/css/term-meta.css' );
			wp_enqueue_script( 'term-meta-script', HARU_STARTER_CORE_URL . 'includes/term-meta/js/term-meta.js' );
		}
				...
				switch ( $field[ 'type' ] ) {
					...
					case 'revslider':
					   ...
					   ?>
						...
						<div class="error notice is-dismissible below-h2"><p><?php echo esc_html__('Revolution Slider is not Installed or Activated.', 'haru-starter') ?></p><button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php echo esc_html__('Dismiss this notice.', 'haru-starter') ?></span></button></div>
						<?php
						}
					...
					case 'icon':
					   ...
					   ?>
						<select name="<?php echo $field[ 'id' ] ?>" id="<?php echo $field[ 'id' ] ?>" class="<?php echo $css_class  ?>" <?php echo $field[ 'multiple' ] ?>>
					    <option value=""><?php __('-- Choose Icon --', 'haru-starter') ?></option>
						<?php
						foreach ( $field[ 'options' ] as $key => $option ) {
							echo '<option' . selected( $field[ 'value' ], $key, FALSE ) . ' value="' . $key . '">' . $option . '</option>';
						}
					...
					case 'image':
						...
						<div class="meta-image-field-wrapper">
							...
							<div style="line-height: 60px;">
								...
								<button type="button"
								        class="haru_upload_image_button button button-primary button-small"><?php _e( 'Upload / Add image', 'haru-starter' ); ?></button>
								<button type="button" style="<?php echo (empty($field[ 'value' ])?'display:none':'') ?>"
								        class="haru_remove_image_button button button-danger button-small"><?php _e( 'Remove image', 'haru-starter' ); ?></button>
							</div>
		...
		public function haru_placeholder_img_src() {
			return HARU_STARTER_CORE_URL . 'includes/term-meta/images/placeholder.png';
		}
