...
( function( window, document, $, app, undefined ) {
	...
	app.SearchView = window.Backbone.View.extend({
		...
		selectPost: function( evt ) {
			...
			/*var current_group = $('.cmb-repeatable-grouping.vidorev-search-post-in-group[data-iterator]');
			if(current_group.length > 0){
				$lastRow = current_group.find( '.attached-posts-wrap .retrieved li' ).last();
			}*/
			
			var page_current_g = $('.attached-posts-wrap.vidorev-search-post-in-group');
	...
	app.openSearch = function( $button ) {
		...
		/*$('.cmb-repeatable-grouping[data-iterator]').removeClass('vidorev-search-post-in-group');
		var parent_group = $button.parents('.cmb-repeatable-grouping[data-iterator]');
		if(parent_group.length > 0){
			parent_group.addClass('vidorev-search-post-in-group');
		}*/
		
		$('.attached-posts-wrap').removeClass('vidorev-search-post-in-group');
		if($('.attached-posts-wrap').length > 1){
			var page_parent_g = $button.parents('.attached-posts-wrap');
			if(page_parent_g.length > 0){
				page_parent_g.addClass('vidorev-search-post-in-group');
			}
		}
