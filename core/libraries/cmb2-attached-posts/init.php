...
class WDS_CMB2_Attached_Posts_Field {
	...
	public function render( $field, $escaped_value, $object_id, $object_type, $field_type ) {
		...
		if ( $this->field->options( 'filter_boxes' ) ) {
			$filter_boxes = '<div class="search-wrap"><input type="text" placeholder="' . sprintf( __( 'Filter %s', 'haru-starter' ), $post_type_labels ) . '" class="regular-text search" name="%s" /></div>';
		}
		...
		echo '<h4 class="attached-posts-section">' . sprintf( __( 'Available %s', 'haru-starter' ), $post_type_labels ) . '</h4>';
		...
		if ( ! $query_users ) {
			$findtxt = $field_type->_text( 'find_text', __( 'Search', 'haru-starter' ) );
		...
		echo '<h4 class="attached-posts-section">' . sprintf( __( 'Attached %s', 'haru-starter' ), $post_type_labels ) . '</h4>';
